...
<form onSubmit={event => {
        setUrl(
           `https://api.themoviedb.org/3/search/person?api_key=${process.env.REACT_APP_MOVIE_API_KEY}&language=en-US&page=1&include_adult=false&query=${query}`
        );
...
{data.results  && data.results.map((result, peopleIndex) => (
  ...
  <div key={peopleIndex} className="column  is-6 m-1 box " >
